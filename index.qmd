...
tax_contributions <- read_xlsx("23dbs01t05co.xlsx", skip = 6, col_names = c("State", "2", "3", "TotalIndividual", "5", "6", "7", "8", "9", "10", "11", "12")) |>
  select(State, TotalIndividual) |>
  slice(c(1:51, 53)) |>
...
states_with_data <- left_join(states_with_data, tax_contributions[, c("NAME", "percent_of_individual_federal_taxes_contributed")])

population <- read_xlsx("NST-EST2024-POP.xlsx", skip = 9, col_names = c("State", "est", "2020", "2021","2022","2023", "est_population")) |>
  select(State, est_population) |>
  slice(1:52) |>
  filter(!is.na(State)) |>
  mutate(
    NAME = toupper(State),
...
states_with_data <- left_join(states_with_data, population[, c("NAME", "percent_of_pop")])
